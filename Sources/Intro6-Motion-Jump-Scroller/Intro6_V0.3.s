...
initFont	move.l	#FontPlane+5*40*4,d0			; TST
...
DoScroller	move	hold(pc),d0
	beq.b	MoveScrollerUp
	subq	#1,hold
	rts
...
ScrollText
	...
	dc.b	1,1
	dc.b	`We can't get another`,1
	dc.b	`   world for free`,1
	dc.b	1,1
	...
	dc.b	`        lives`,1
	dc.b	1
	dc.b	`   An earthquake`,1
	dc.b	`   in Los Angeles`,1
	dc.b	1
	dc.b	`A world disaster in`,1
	dc.b	`   full progress`,1
	dc.b	1
	dc.b	`Rainforests burning`,1
	dc.b	`it's all screwed up`,1
	dc.b	1
	dc.b	`A greenhouse effect`,1
	dc.b	`   is not to stop`,1
	dc.b	1
	dc.b	`We know and see but`,1
	dc.b	`    don't react`,1
	dc.b	1
	dc.b	`We don't face the`,1
	dc.b	`    hard facts`,1
	dc.b	1
	dc.b	`  Too many people`,1
	dc.b	`  too little space`,1
	dc.b	1
	dc.b	`  For mankind it's`,1
	dc.b	`  almost too late`,1
	dc.b	1
	dc.b	`We can't get another`
	dc.b	`   world for free`,1
	...
	DC.B	` I NEED A SONG FOR`,1
	DC.B	`  THIS URRG-INTRO`,1,1,1
	DC.B	`SPECIAL: A SONG WITH`,1
	DC.B	`120 BPM FOR SCROLL-`,1,1
	DC.B	` JUMP-TIMING AND A`,1,1
	DC.B	`COOL DESIGNED BACK-`,1,1
	DC.B	` GROUND COLOR WEDGE`,1,1
	dc.b	1,1,1
	dc.b	`TIME SPEND:`,1
	dc.b	`           ABOUT 5h`,1
	dc.b	1,1,1
