...
.copyDS9toScreen
	lea	FWPic,a0
	lea	DS9Screen+80+7+60*160,a1
	bsr.b	.copy
	lea	FWPic+48*66,a0
	lea	DS9Screen+80+7+130*160,a1
	bsr.b	.copy
	bra.w	VbiEnd
.copy	wblt
	move	#0,$64(a6)
...
TyperText:	; Usage: 0=End 1=Return
 dc.b 1
 DC.B 1
 DC.B `              YEEAAH MATES IT IS TIME TO CALL THE ONLY WAY`,1
 DC.B `              --------------------------------------------`,1
 dc.b	0
TyperText2:
 DC.B 1
 ...
 DC.B 1
 DC.B `              /---------------------------------------------/`,1
 DC.B `             /  FEEL FREE TO CALL THE REAL  - FAST WAY -   /`,1
 DC.B `            /---------------------------------------------/`,1
 DC.B 1
 DC.B 1
 ...
 DC.B 1
 DC.B `                                MOTION GHQ`,1
 DC.B `                        SYSOPS: ANDY/MTN DONALD/MTN`,1
 DC.B 1
 DC.B 0,0,0,0
...
ScrollText:
	...
	DC.B	` THE GFX WERE DONE BY DUKE.    `
	DC.B	` THE SOUND WAS COMPOSED BY  .... ? `
	DC.B	`                          `
	DC.B	`    A WHOLE INTRO WITH 3 LOGOS, 1 FONT, CHIPTUNE,`
...
FWPic:		incbin	FastWayAddtro/FastWay528x96x1.raw
		cnop	0,8
